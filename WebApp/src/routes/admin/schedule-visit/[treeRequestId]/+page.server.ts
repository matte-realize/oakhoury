...
export const actions: Actions = {
	default: async ({ request, locals, fetch, params }) => {
		// Auth check again for action
		if (!locals.user) {
			throw error(401, 'Not authenticated');
		}
		const organizationMemberId = locals.user.id;
		...
		const apiUrl = `${API_ROUTE}/schedule-visit`;
		...
		if (!response.ok) {
			...
			console.error(`API Error (${response.status}) scheduling visit: ${errorBody}`);
			return message(form, `Failed to schedule visit. Server responded: ${response.statusText}`, {
				status: 500
			});
		}
